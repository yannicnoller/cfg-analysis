...
public class CFG implements Serializable {
	...
	/**
	 * Gets list of edges which originate on a given node or which are incident on a
	 * node.
	 */
	...
	/**
	 * Adds a node to the CFG, creating an entry in the offset map so the node can
	 * be retrieved by its start offset later.
	 */
	...
	/**
	 * Returns the name of the method with which this control flow graph is
	 * associated.
	 *
	 */
	...
	/**
	 * Computes the distance in terms of number of edges from one node to another
	 * node
	 * 
	 * @return Integer.MAX_VALUE if not reachable the least number of edges
	 *         otherwise
	 */
	...
	/**
	 * Returns string representation of the control flow graph, which is a list of
	 * the edges that constitute the CFG.
	 *
	 */
	...
	/**
	 * Gets string representation of the edges that compose the control flow graph
	 * for this method.
	 */

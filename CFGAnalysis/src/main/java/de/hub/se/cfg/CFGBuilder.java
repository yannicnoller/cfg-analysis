...
/**
 * This class builds the control flow graph. Implementation based on jpf-memoize
 * by Guowei Yang (guoweiyang@utexas.edu).
 * 
...
public class CFGBuilder {
	...
	private CFG buildCFG(int methodIndex) throws Exception {
		...
		if (completeMethodName == null) {
			/*
			 * completeMethodName might be null for abstract or native methods, in such a
			 * case we don't want to or cannot build a cfg
			 */
			return null;
		}
	...
	/**
	 * Retrieves a control flow graph for a method from the cache using a complete
	 * method name
	 * 
	 */
	...
	private void formNodes(CFG cfg, InstructionList il, LineNumberTable lineNumberTable) {
		...
		while (leadersIt.hasNext() && endIt.hasNext()) {
			...
			/*
			 * Create new node, use min and max calculation for sourcecode line because they
